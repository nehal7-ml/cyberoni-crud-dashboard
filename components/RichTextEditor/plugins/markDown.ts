...
function markdown(src: string) {
  ...
  function replace(
    rex: RegExp | string,
    replace: ((sub: string, ...arg: any[]) => string) | string,
  ) {
      ...
      element(
        ...
        table.replace(rx_row, function (row: string, ri: any) {
          return row == sep
            ...
            : element(
                "tr",
                row.replace(rx_cell, function (all: any, cell: any, ci: any) {
                  return ci
                    ? element(
                        sep && !ri ? "th" : "td",
  ...
  // stash
  replace(rx_stash, function (all: string) {
...
export function markdownPlugin(editor: Editor) {
  ...
  editor.ui.registry.addButton("markdown", {
    ...
    onAction: function () {
      ...
      newModal.show();
      document
        .getElementById("parse-markdown")
        ?.addEventListener("click", () => {
          const markdownRaw = (
            document.getElementById("markdown") as HTMLTextAreaElement
          ).value;
          editor.setContent("<pre>" + markdown(markdownRaw) + "</pre>", {
            format: "raw",
          });
          newModal.close();
        });
      document
        .getElementById("markdown-close")
        ?.addEventListener("click", () => {
          newModal.close();
        });
      //
